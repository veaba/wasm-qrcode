...
fastify.get('/qrcode', async (request, reply) => {
  const { text = 'https://example.com', size = 256, type = 'svg' } = request.query;
...
// 使用
const stream = createQRCodeStream('https://example.com', 256);
...
// 第一次生成（计算）
const svg1 = getQRCodeSVG('https://example.com');

// 第二次生成（缓存，快 10-100 倍）
const svg2 = getQRCodeSVG('https://example.com');
```
