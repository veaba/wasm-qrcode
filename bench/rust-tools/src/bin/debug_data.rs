...
use qrcode_rust::{
    get_rs_blocks, QRCode as QRCodeRust, QRCodeOptions, QRErrorCorrectLevel as LevelRust,
};
...
fn main() {
    ...
    if let Some(ref data) = qr.data_cache {
        ...
        println!(
            "  字节0低4位 + 字节1高4位 (长度): {:04b} {:04b} = {}",
            len_high, len_low, len
        );
