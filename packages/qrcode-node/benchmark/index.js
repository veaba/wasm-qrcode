 ...
 * 单条生成测试
 */
function benchmarkSingleGeneration() {
  const results = [];
 ...
 * 批量生成测试
 */
function benchmarkBatchGeneration() {
  const results = [];
 ...
 * 不同输出格式测试
 */
function benchmarkOutputFormats() {
  const results = [];
 ...
 * 不同纠错级别测试
 */
function benchmarkErrorCorrectionLevels() {
  const results = [];
  const text = TEST_TEXTS.medium;
  const levels = [
 ...
 * 异步生成测试
 */
async function benchmarkAsyncGeneration() {
  const results = [];
  const text = TEST_TEXTS.medium;
