...
function benchmarkSingle() {
  const text = 'https://github.com/veaba/qrcodes';
...
function benchmarkBatch() {
  const texts = Array.from({ length: BENCHMARK_ITERATIONS }, (_, i) =>
    `https://github.com/veaba/qrcodes/${i}`
  );
...
function benchmarkVariousLengths() {
  ...
  const testCases = [
    ...
    { name: 'Medium (50 chars)', text: 'https://github.com/veaba/qrcodes/issues/123' },
    { name: 'Long (100 chars)', text: 'https://github.com/veaba/qrcodes/issues/123/comments/456'.padEnd(100, 'a') },
    { name: 'Very Long (200 chars)', text: 'https://github.com/veaba/qrcodes'.padEnd(200, 'x') }
  ];
...
// 主函数
function main() {
  console.log('🚀 @veaba/qrcode-bun Benchmark');
  ...
  // 保存到文件
  Bun.write('packages/qrcode-bun/benchmark_result.json', JSON.stringify(results, null, 2));
}
